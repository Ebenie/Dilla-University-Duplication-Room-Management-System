...
$dbname = "duplicationmgmt";
    ...
    if ($action == "approve") {
        $sql = "UPDATE request SET duplication_approval ='approved' WHERE id=?";
    } elseif ($action == "reject") {
        $sql = "UPDATE request SET duplication_approval='rejected', req_status='rejected' WHERE id=?";
    } else {
        echo "Invalid action.";
        exit;
    }
    ...
    // Execute the query
    if ($stmt->execute()) {
        header("Location: duplicationHome.php");
    } else {
